...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('SELECT last_value, is_called FROM %I.%I',
                      source_schema, object.sequence_name) INTO seq_info;

        EXECUTE format('CREATE SEQUENCE %I.%I', dest_schema, object.sequence_name);

        -- If the sequence has been used, set its current value
        IF seq_info.is_called THEN
            EXECUTE format('SELECT setval(%L, %s, true)',
                          dest_schema || '.' || object.sequence_name, seq_info.last_value);
        ELSE
